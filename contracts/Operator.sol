...
import "./AuthorizedRequester.sol";
    ...
    WithdrawalInterface
{
     ...
     * @param theOwner The address of the owner
     */

    function __Operator_init(
        address link,
        address theOwner
    ) public initializer {
        __Operator_init_unchained(link, theOwner);
    }

    function __Operator_init_unchained(
        address link,
        address theOwner
    ) internal onlyInitializing {
        linkToken = LinkTokenInterface(link);
        __ConfirmedOwner_init(theOwner);
    }
    ...
    function oracleRequest(
        ...
        external
        override
        validateAuthorizedRequester(callbackAddress)
        validateFromLINK
    {
        (
            bytes32 requestId,
    ...
    function operatorRequest(
        ...
        uint256 dataVersion,
        bytes calldata data
    ) external override validateAuthorizedRequester(sender) validateFromLINK {
        (
            bytes32 requestId,
            uint256 expiration
        ) = _verifyAndProcessOracleRequest(
     ...
     * @param targets The addresses to set permissions on
     * @param requesters The addresses that are allowed to send updates
     */
    function setAuthorizedRequestersOn(
        address[] calldata targets,
        address[] calldata requesters
    ) public validateAuthorizedRequesterSetter {
        emit TargetsUpdatedAuthorizedSenders(targets, requesters, msg.sender);
     ...
     * @return bool of whether sender is authorized
     */
    function _canSetAuthorizedRequesters()
        internal
        view
        override
        returns (bool)
    {
        return isAuthorizedRequester(msg.sender) || owner() == msg.sender;
    }
